...
public class SolicitationServiceImpl implements SolicitationService {
    ...
    public Flux<Solicitation> findByTitle(String title) {
        return this.solicitationRepository.findByAuthorAndDeleteIsFalse(title).switchIfEmpty(
                Mono.error(new Exception("No Solicitation found with title containing: " + title)));
    }
